...
% Can optimise the band choices for the MI function, and M
t0 = tic();
...
bp = 4:4:40; 
[S1,S2,S3,S4,S5,S6,S7,S8] = preproc(P1,P2,P3,P4,P5,P6,P7,P8,bp);
...
[x_train1,y_train1] = fbcsp(S1([1:s],:),2,bp);
[x_train2,y_train2] = fbcsp(S2([1:s],:),2,bp);
[x_train3,y_train3] = fbcsp(S3([1:s],:),2,bp);
[x_train4,y_train4] = fbcsp(S4([1:s],:),2,bp);
[x_train5,y_train5] = fbcsp(S5([1:s],:),2,bp);
[x_train6,y_train6] = fbcsp(S6([1:s],:),2,bp);
[x_train7,y_train7] = fbcsp(S7([1:s],:),2,bp);
[x_train8,y_train8] = fbcsp(S8([1:s],:),2,bp);
...
[x_test1,y_test1] = fbcsp(S1([s:end],:),2,bp);
[x_test2,y_test2] = fbcsp(S2([s:end],:),2,bp);
[x_test3,y_test3] = fbcsp(S3([s:end],:),2,bp);
[x_test4,y_test4] = fbcsp(S4([s:end],:),2,bp);
[x_test5,y_test5] = fbcsp(S5([s:end],:),2,bp);
[x_test6,y_test6] = fbcsp(S6([s:end],:),2,bp);
[x_test7,y_test7] = fbcsp(S7([s:end],:),2,bp);
[x_test8,y_test8] = fbcsp(S8([s:end],:),2,bp);
...
function [miTrain,y_train] = fbcsp(S1,m,bp) % Following paper "Common Spatial Pattern Method for Channel Selection in
...
featL = zeros(size(S1,1),size(S1,2)*m*m);
featR = zeros(size(S1,1),size(S1,2)*m*m);
for i = 1:size(S1,2)
    for j = 1:size(S1,1)
...
function [S1,S2,S3,S4,S5,S6,S7,S8] = preproc(P1,P2,P3,P4,P5,P6,P7,P8,bp)
